...
const [isBatchAdding, setIsBatchAdding] = useState(false);
  ...
  const handleSlotFilesSelected = async (event: React.ChangeEvent<HTMLInputElement>, slotId: string) => {
      ...
      setCandidates(prev => prev.map(c => c.slotId === (slotId as any) ? { ...c, isLoadingFiles: true } : c));
      ...
      if (processedFiles.length === 0) {
          ...
          setCandidates(prev => prev.map(c => c.slotId === (slotId as any) ? { ...c, candidateName: "", isLoadingFiles: false } : c));
          return;
      }
      
      setCandidates(prev => prev.map(c => {
          if (c.slotId === (slotId as any)) {
  ...
  const handleRepoFileSelect = async (repoFiles: RepositoryFile[]) => {
      try {
          ...
          if (typeof repoPickerTarget === 'string' && ['regulation', 'form', 'misc'].includes(repoPickerTarget)) {
              ...
              setCandidates(prev => prev.map(c => c.slotId === (repoPickerTarget as any) ? { ...c, isLoadingFiles: true } : c));
          }
          ...
          if (typeof repoPickerTarget === 'string' && ['regulation', 'form', 'misc'].includes(repoPickerTarget)) {
              ...
              setCandidates(prev => prev.map(c => c.slotId === (repoPickerTarget as any) ? { ...c, isLoadingFiles: false } : c));
          }
      }
  };
                          ...
                          <div className="flex items-center gap-3">
                              <button 
                                  onClick={async () => {
                                      try {
                                          ...
                                          alert("Configurações do edital salvas com sucesso!");
                                      } catch (e) {
                                          console.error("Failed to save edital settings", e);
                                          alert("Erro ao salvar as configurações do edital.");
                                      }
                                  }}
                              ...
                              <button 
                                  onClick={() => {
                                      ...
                                      saveEditalSettings(context.editalTitle, context).catch(e => console.warn("Failed to save edital settings", e));
                                  }}
                                  disabled={!context.regulationText}
