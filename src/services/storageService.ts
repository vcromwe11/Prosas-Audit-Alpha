...
const sanitizeDocId = (id: string) => {
  ...
  return id.replace(/[\/\\]/g, '_').replace(/[\r\n]+/g, ' ').trim().substring(0, 150);
};

export const saveEditalSettings = async (editalName: string, context: Omit<AuditContext, 'isReady'>) => {
  if (!auth.currentUser) throw new Error("Requires authentication");
  if (!editalName || editalName.trim() === '') return;
  
  const safeId = sanitizeDocId(editalName);
  const path = `edital_settings/${safeId}`;
  try {
    const docRef = doc(db, 'edital_settings', safeId);
    await setDoc(docRef, {
      id: editalName,
      context,
...
export const getEditalSettings = async (editalName: string): Promise<EditalSettings | null> => {
  if (!auth.currentUser) return null;
  const safeId = sanitizeDocId(editalName);
  const path = `edital_settings/${safeId}`;
  try {
    ...
    const docSnap = await getDoc(docRef);
    if (docSnap.exists()) {
      return docSnap.data() as EditalSettings;
    }
    return null;
  } catch (error) {
    handleFirestoreError(error, OperationType.GET, path);
