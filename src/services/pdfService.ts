 ...
 */
export const extractTextFromPdf = async (file: File, asMarkdown: boolean = false): Promise<string> => {
  ...
  if (asMarkdown) {
    try {
      const arrayBuffer = await file.arrayBuffer();
      // Use standard Uint8Array conversion for pdf2md
      const uint8Array = new Uint8Array(arrayBuffer);
      const markdown = await pdf2md(uint8Array);
      return markdown;
    } catch (e) {
      console.warn("pdf2md failed, falling back to raw text extraction", e);
    }
  }
